...
The primary objective of this report was to develop a reliable model for forecasting yearly mean sunspot numbers from 1700 to 2023. Through a comprehensive analysis that included exploratory data analysis, stationarity and normality tests, data transformation, and thorough model specification, we aimed to capture the underlying patterns and trends in the sunspot data effectively.

Our initial exploratory data analysis revealed significant seasonality in the sunspot series, corresponding to the well-known 11-year solar cycle. This necessitated the use of seasonal models, specifically the Seasonal AutoRegressive Integrated Moving Average (SARIMA) models. After identifying the seasonal orders, we focused on determining the optimal non-seasonal AR and MA orders through tools such as ACF and PACF plots, EACF tables, and BIC tables.

We evaluated a set of 15 candidate SARIMA models, estimating their parameters and assessing their goodness-of-fit through various criteria, including AIC, BIC, and error measures (ME, RMSE, MAE, MPE, MAPE, MASE). The analysis revealed that higher-order models like SARIMA(6,1,8)(1,1,2)[11] and SARIMA(6,1,10)(1,1,2)[11] performed exceptionally well across multiple metrics, capturing the series' autocorrelation structure and exhibiting near-normal residuals.

However, given the complexity and over-parameterization of some models, we selected the SARIMA(6,1,8)(1,1,2)[11] model as the optimal choice. This model strikes a balance between performance and simplicity, providing robust forecasts while avoiding unnecessary complexity. It demonstrated the highest AIC score and ranked fifth in BIC score, with a marginal difference from simpler models.

The over-parameterized neighboring models, such as SARIMA(6,1,9)(1,1,2)[11], did not show significant improvements over the SARIMA(6,1,8)(1,1,2)[11] model across most evaluation metrics, further justifying our selection.

In conclusion, the SARIMA(6,1,8)(1,1,2)[11] model was chosen as the optimal model for forecasting future sunspot numbers. This model effectively captures the seasonal and non-seasonal patterns in the data, providing accurate and reliable forecasts. The resulting model was used to forecast sunspot numbers for the next 10 years, from 2024 to 2033, offering valuable insights for understanding solar cycles and their potential implications for space weather and climate.
...
## Comparison of Error measure scores (Visual Aid)

In the figure below, areas highlighted in green denote the lowest scores on the error measures, while those highlighted in yellow signify the second lowest scoresand the red color signifies the third lowest.

![](./Images/Error_scores_table.png)

## Utility Function - `sort.score()`
...
The `sort.score()` function facilitates the sorting of models based on either the Akaike Information Criterion (AIC) or the Bayesian Information Criterion (BIC) scores. This function is particularly useful for comparing and selecting models based on their AIC or BIC scores, aiding in the model selection process by identifying the models with the best goodness-of-fit and balance of complexity.

## Utility Function - `arima_summary()`
...
The `arima_summary()` function generates parameter estimation results for ARIMA/SARIMA models using three different estimation methods: Least Squares (CSS), Maximum Likelihood (ML), and a combination of both (CSS-ML). It takes time series data (`TS_data`) and the desired ARIMA order (`pdq_order`), seasonal order (`seasonal_order`) and period (`period`) as input.

The function first checks if the input data is of class 'ts', then estimates ARIMA models using CSS, ML, and CSS-ML methods. It prints the parameter estimation results for each method and stores the models in a list with appropriate names based on the ARIMA order. Finally, it returns the list of models for further analysis.

This function facilitates the comparison of parameter estimation results obtained through different estimation techniques, providing valuable insights into the stability and reliability of the fitted ARIMA models.

## Utility Function - `plot_acf_pacf()`
...
This function is utilized in the report to visualize the ACF and PACF of the time series data, providing insights into the autocorrelation and partial autocorrelation structures. These plots are essential for identifying potential ARIMA model parameters by revealing significant lags that indicate autoregressive (AR) and moving average (MA) components in the data.

## Utility Function - `check_residuals()`

```{r}
# Plotting residual diagnostic plots - TS plot, ACF, PACF, Histogram, QQPlot, Ljung Test Plot
...
The check_residuals function generates a comprehensive set of diagnostic plots for analyzing the residuals of a fitted ARIMA/SARIMA model. It fits an ARIMA model to the provided time series data, computes standardized residuals, and prints the Shapiro-Wilk test result for normality of residuals. 

The function then creates six plots in a 2x3 grid layout: a residual plot over time, a histogram of residuals, a QQ plot to assess normality, an autocorrelation function (ACF) plot, a partial autocorrelation function (PACF) plot, and a Ljung-Box test plot to check for remaining autocorrelation. Finally, it resets the plotting area configuration to the default setting. This function is essential for validating ARIMA/SARIMA model assumptions by examining residuals for any patterns or anomalies.
